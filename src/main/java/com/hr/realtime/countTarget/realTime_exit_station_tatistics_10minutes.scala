  ...
  def main(args: Array[String]): Unit = {
   ...
   //1. 从kafka实时消费数据
    val conf: SparkConf = new SparkConf()
      ...
      .setMaster("local[*]")  //local[*],yarn-cluster

   // conf.set("spark.driver.allowMultipleContexts","true")

    val ssc = new StreamingContext(conf, Seconds(5)) //Seconds ,Minutes(10)
    ...
    spark.udf.register("getMinutesPeriod", Unit  => getMinutesPeriod())



    //从kafka获取车道出口交易数据数据流
    val ETC_TollEx_BillInfo_DStream_Original = MySeniorKafkaUtil.getKafkaStream(ssc, DataBaseConstant.TOPIC_ETC_TollEx_BillInfo, groupId)
...
//    })
    ...
    val resultTableName= "ETC_exConsumeDailyStatistic"  //测试表名
    ...
    ETC_TollEx_BillInfo_WindowsDStream.foreachRDD{rdd=>{
      ...
      rdd.map(value=>{
        println(s"---${value}---")
        JSON.parseObject(value, classOf[EtcTollexBillInfo])
      }).toDF().createOrReplaceTempView(s"${originTableName}")
      ...
      val exitStationTatisticsSql =
        s"""
           ...
           |current_timestamp()    statisticDay , --comment 当做进行统计生时间
