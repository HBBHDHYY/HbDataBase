...
/**
  ...
  * 2020-06-05 11:13
  */
object realTime_etc_join_viu_tellRow2 {
    ...
    var etc_join_viu: DStream[(String, String, String, String, String, String, Int, Int)] = etcWindowsGantryidDStream.cogroup(viuWindowsGantryidDStream).flatMap(
      t => {
        ...
        val viuMembers = viuIter.toList.map(x => {
          (x.getAs[String](0), x.getAs[String](1), x.getAs[String](2), x.getAs[Long](3), x.getAs[String](4))}) //getString方法不行,why?get(5)可以
...
//    写入kafka的例子
//    etc_join_viu.foreachPartition(rdd=>{
//      rdd.foreach(record=>{
